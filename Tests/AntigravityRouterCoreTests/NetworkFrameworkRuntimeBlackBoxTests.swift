...
final class NetworkFrameworkRuntimeBlackBoxTests: XCTestCase {
    ...
    func testMonitorHelpIsBounded() throws {
        ...
        XCTAssertTrue(text.contains("usage: AntigravityRouterMonitor"))
    }

    private func monitorExecutable() throws -> String {
        let cwd = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        let candidates = [
            cwd.appendingPathComponent(".build/debug/AntigravityRouterMonitor").path,
            cwd.appendingPathComponent(".build/arm64-apple-macosx/debug/AntigravityRouterMonitor").path,
            cwd.appendingPathComponent(".build/x86_64-apple-macosx/debug/AntigravityRouterMonitor").path
        ]
        ...
        build.arguments = ["swift", "build", "--product", "AntigravityRouterMonitor"]
        try build.run()
        build.waitUntilExit()
        if let existing = candidates.first(where: { FileManager.default.isExecutableFile(atPath: $0) }) {
            return existing
        }
        throw XCTSkip("AntigravityRouterMonitor executable not found after build")
    }
