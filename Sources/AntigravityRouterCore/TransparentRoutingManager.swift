...
public enum TransparentRoutingScript {
    private static let pfAnchorName = "com.antigravityrouter"
    private static let pfAnchorFile = "/etc/pf.anchors/com.antigravityrouter"
    ...
    public static func enable(proxyPort: Int) -> String {
        ...
        token_file="/etc/pf.anchors/com.antigravityrouter.token"
        pf_conf="\(pfConfFile)"
        pf_backup="/etc/pf.conf.antigravityrouter.bak"
        hosts_backup="$(mktemp)"
        hosts_tmp="$(mktemp)"
        anchor_tmp="$(mktemp)"
        tmp_pf="$(mktemp)"
        ...
        awk 'BEGIN{skip=0} $0=="# AntigravityRouter START"{skip=1;next} $0=="# AntigravityRouter END"{skip=0;next} skip==0{print}' /etc/hosts > "$hosts_tmp"
        cat >> "$hosts_tmp" <<'EOF'
        # AntigravityRouter START
        \(hostLines)
        # AntigravityRouter END
        EOF
        cat > "$anchor_tmp" <<'EOF'
        ...
        $0=="# AntigravityRouter PF START"{skip=1;next}
        $0=="# AntigravityRouter PF END"{skip=0;next}
        skip==1{next}
        {
          print
          if (inserted==0 && $0 ~ /^rdr-anchor[ \t]/) {
            print "# AntigravityRouter PF START"
            print "rdr-anchor \\"com.antigravityrouter\\""
            print "# AntigravityRouter PF END"
            inserted=1
          }
        }
        END{
    ...
    public static func disable() -> String {
        ...
        set -e
        tmp="$(mktemp)"
        awk 'BEGIN{skip=0} $0=="# AntigravityRouter START"{skip=1;next} $0=="# AntigravityRouter END"{skip=0;next} skip==0{print}' /etc/hosts > "$tmp"
        cat "$tmp" > /etc/hosts
        rm -f "$tmp"
        pf_conf="\(pfConfFile)"
        tmp_pf="$(mktemp)"
        awk 'BEGIN{skip=0} $0=="# AntigravityRouter PF START"{skip=1;next} $0=="# AntigravityRouter PF END"{skip=0;next} skip==0{print}' "$pf_conf" > "$tmp_pf"
        /sbin/pfctl -nf "$tmp_pf"
        cat "$tmp_pf" > "$pf_conf"
        rm -f "$tmp_pf"
        /sbin/pfctl -a com.antigravityrouter -F all >/dev/null 2>&1 || true
        if [ -f /etc/pf.anchors/com.antigravityrouter.token ]; then
          token="$(awk '/Token/ {print $3; exit}' /etc/pf.anchors/com.antigravityrouter.token)"
          if [ -n "$token" ]; then /sbin/pfctl -X "$token" >/dev/null 2>&1 || true; fi
        fi
        rm -f /etc/pf.anchors/com.antigravityrouter
        rm -f /etc/pf.anchors/com.antigravityrouter.token
        /sbin/pfctl -f "$pf_conf" >/dev/null 2>&1 || true
        /usr/bin/dscacheutil -flushcache >/dev/null 2>&1 || true
        /usr/bin/killall -HUP mDNSResponder >/dev/null 2>&1 || true
        """
