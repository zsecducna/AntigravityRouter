...
## Goal
Build `AntigravityPorter`, a local trusted HTTPS proxy that lets Google Antigravity keep its native workflow while selected model traffic is executed through `cheaprouter.uk`.
...
- Listener-only app-env routing for Antigravity: AntigravityPorter launches Antigravity with `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and lowercase equivalents pointed at the local listener.
...
8. As a user, enabling/disabling AntigravityPorter never changes my macOS global proxy settings.
