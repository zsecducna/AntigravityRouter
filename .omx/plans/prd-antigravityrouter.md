# PRD: AntigravityRouter

## Metadata
- Date: 2026-05-03T17:17:24Z
- Source spec: `.omx/specs/deep-interview-antigravityrouter.md`
...
## Goal
Build `AntigravityRouter`, a local trusted HTTPS proxy that lets Google Antigravity keep its native workflow while selected model traffic is executed through `cheaprouter.uk`.

## Non-Negotiable V1 Definition
V1 is not a demo adapter. V1 is daily-driver behavior parity for captured real Antigravity workflows.
...
## Users
- Primary user: owner/operator who uses Google Antigravity and wants selected models routed through cheaprouter.
...
- Listener-only app-env routing for Antigravity: AntigravityRouter launches Antigravity with `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and lowercase equivalents pointed at the local listener.
- No normal-mode mutation of macOS Secure Web Proxy or Auto Proxy settings; PAC remains diagnostic fallback only.
- CONNECT handling for Google inference hosts.
- TLS termination only for allowed inference endpoints.
- All HTTPS CONNECT hosts are blind-tunneled by default in app-env mode. Known inference hosts are logged separately for later MITM/translation. Auth/userinfo hosts are never TLS-terminated.
...
- Translation from captured Antigravity request shapes to cheaprouter-compatible requests.
...
2. As a user, I can install and trust the AntigravityRouter CA using guided steps and see verification status.
3. As a user, I can route specific models through cheaprouter while unknown models default to Google direct.
4. As a user, I can capture real Google-backed Antigravity workflows before routing them.
...
8. As a user, enabling/disabling AntigravityRouter never changes my macOS global proxy settings.

## Acceptance Criteria
- Captured real Antigravity workflows pass end-to-end through AntigravityRouter for routed models.
...
- Normal-mode enable leaves `HTTPSEnable` and `ProxyAutoConfigEnable` unchanged in `scutil --proxy`.
- Env-launched Antigravity opens `language_server_macos_arm` sockets to `127.0.0.1:8877`.
- Known inference hosts are visible in the Log tab before MITM/translation is enabled.
- cheaprouter requests never loop through the local routing path.
...
- Global all-HTTPS proxy and PAC routing were rejected as default routing paths after live testing: global proxy can break the whole Mac, and Antigravity model workers ignore macOS PAC.
...
5. Integration gate: live Antigravity workflows pass on a local Mac.
